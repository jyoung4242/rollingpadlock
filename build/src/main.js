...
const model = {
    launch: (event, model) => {
        ...
        model.combolock.numOfTries.forEach((t) => (t.status = true));
    ...
    combolock: {
        ...
        onLoad: () => {
            ...
            if (model.combolock.numOfTargets == 3)
                model.combolock.winningAnswer = chance.integer({ max: 999, min: 0 });
            else if (model.combolock.numOfTargets == 4)
                model.combolock.winningAnswer = chance.integer({ max: 9999, min: 0 });
            else
                model.combolock.winningAnswer = chance.integer({ max: 99999, min: 0 });
        },
        numUserGuesses: 0,
        dials: [],
        timeremaining: 30,
        ...
        guessLock: false,
        submitGuess: (_e, model) => {
            if (model.combolock.guessLock)
                return;
